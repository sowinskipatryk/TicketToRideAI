...
class NetworkAdapter(BlankAdapter):
    ...
    def __init__(self, game):
        super().__init__()
        self.game = game
        self.train_colors_num = len(self.game.config.TRAIN_COLORS)

        self.common_values = ((([self.VALUE_NEUTRAL] * self.game.config.NUM_FACE_UP_CARDS * (self.train_colors_num + 1)
                                + [self.VALUE_POSITIVE] * 3)
                               + [self.VALUE_POSITIVE] * self.game.players_num)
                              + [self.VALUE_NEUTRAL] * self.game.board.get_route_links_num() * self.game.players_num)

        self.player_values = [([self.VALUE_NEUTRAL]
                               + [self.VALUE_NEUTRAL] * self.train_colors_num
                               + [self.VALUE_NEUTRAL] * (self.game.ticket_deck.get_tickets_num() * 2)
                               ) for _ in range(self.game.players_num)]

        self.face_up_cards_pos = 0
        self.draw_pile_num_pos = self.game.config.NUM_FACE_UP_CARDS * (self.train_colors_num + 1)
        ...
        self.route_owner_pos = self.trains_num_pos + self.game.players_num
        ...
        self.ticket_completed_pos = self.ticket_owner_pos + self.game.ticket_deck.get_tickets_num()

    def normalize_trains_num(self, trains_num):
        return trains_num / self.game.config.NUM_TRAIN_FIGURES

    def normalize_train_pile_num(self, cards_num):
        return cards_num / ((self.game.config.NUM_TRAIN_CARDS_PER_COLOR * self.train_colors_num)
                            + self.game.config.NUM_WILD_CARDS)

    def normalize_ticket_pile_num(self, cards_num):
        return cards_num / self.game.ticket_deck.get_tickets_num()

    def normalize_color_cards_num(self, cards_num):
        return cards_num / self.game.config.NUM_TRAIN_CARDS_PER_COLOR

    def normalize_wild_cards_num(self, cards_num):
        return cards_num / self.game.config.NUM_WILD_CARDS

    def normalize_player(self, player_id):
        return player_id / (self.game.players_num - 1)
    ...
    def set_route_owner(self, player_id, route_id):
        self.common_values[self.route_owner_pos + (route_id * self.game.players_num) + player_id] = self.VALUE_POSITIVE
    ...
    def set_color_cards_num(self, player_id, color, value):
        color_id = self.game.config.TRAIN_COLORS.index(color)
    ...
    def set_ticket_owner(self, player_id, ticket):
        ticket_id = self.game.ticket_deck.get_ticket_id(ticket)
    ...
    def print_human_readable_information(self, player_id):
        ...
        logger.warning(f"table: {self.game.config.TRAIN_COLORS + ['None', 'wild']}")
        logger.warning(f"face up: {self.game.train_card_manager.get_face_up_cards()}")
        for i in range(self.game.config.FACE_UP_CARDS_NUM):
            logger.warning(f'one hot card {i}: {self.common_values[(self.train_colors_num + 1) * i : (self.train_colors_num + 1) * (i + 1)]}')
        for i in range(self.game.board.get_route_links_num()):
            logger.warning(f"route owner: {self.game.board.get_link_owner(i)}")
            logger.warning(f"one hot route owned {i}: {self.common_values[self.route_owner_pos + (i * self.game.players_num):self.route_owner_pos + ((i + 1) * self.game.players_num)]}")
        logger.warning(f"draw pile num: {len(self.game.train_card_manager._draw_pile)}")
        logger.warning(f"draw pile norm: {self.common_values[self.draw_pile_num_pos]}")
        logger.warning(f"discard pile num: {len(self.game.train_card_manager._discard_pile)}")
        logger.warning(f"discard pile norm: {self.common_values[self.discard_pile_num_pos]}")
        logger.warning(f"ticket deck num: {self.game.ticket_deck.tickets_left}")
        logger.warning(f"ticket deck norm: {self.common_values[self.ticket_pile_num_pos]}")
        logger.warning(f"train figures num: {[player.trains_remaining for player in self.game.players]}")
        logger.warning(f"train figures norm: {self.common_values[self.trains_num_pos : self.trains_num_pos + self.game.players_num]}")
        logger.warning(f"hand: {self.game.players[player_id].hand}")
        logger.warning(f"wild cards norm: {[self.player_values[player_id][self.wild_cards_num_pos]]}")
        logger.warning(f"table: {self.game.config.TRAIN_COLORS}")
        ...
        logger.warning(f"tickets owned: {[self.game.ticket_deck.get_ticket_id(ticket) for ticket in self.game.players[player_id].tickets]}")
        logger.warning(f"one hot tickets owned: {self.player_values[player_id][self.ticket_owner_pos:self.ticket_owner_pos + self.game.ticket_deck.tickets_num]}")
    """
